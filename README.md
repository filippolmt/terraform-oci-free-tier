...
This repository provides OpenTofu/Terraform configurations for deploying resources in the Oracle Cloud Infrastructure (OCI) Free Tier. Includes optional support for [RunTipi](https://runtipi.io) homeserver, [Coolify](https://coolify.io) self-hosted PaaS, and [WireGuard](https://www.wireguard.com/) VPN client.
...
2. **Configure your variables**:
    ...
    By default, `install_runtipi = true` installs RunTipi. Set it to `false` to skip. Alternatively, set `install_coolify = true` to install Coolify instead (mutually exclusive — both use ports 80/443).
    ```bash
...
- `scripts/startup.sh`: Cloud-init script with **two-phase architecture**:
    - **Phase A** (cloud-init, fast): Sets system timezone (default: Europe/Rome), installs packages and Docker, configures SSH key, writes Phase B script and systemd unit, exits
...
**Auto-added when `install_coolify = true`:**
- TCP 80 (HTTP)
- TCP 443 (HTTPS)
- TCP 8000 (Coolify UI) — source configurable via `coolify_admin_source_cidr` (default: `0.0.0.0/0`)
- TCP 6001-6002 (Coolify real-time / soketi) — source configurable via `coolify_admin_source_cidr`; needed for IP-based access, can be closed after configuring a custom domain
...
[Coolify](https://coolify.io) is a self-hosted PaaS alternative (like Heroku/Vercel). It is **mutually exclusive** with RunTipi — both use ports 80/443. This constraint is enforced via a `precondition` on the compute instance (hard error at plan/apply).
...
| <a name="input_coolify_admin_email"></a> [coolify\_admin\_email](#input\_coolify\_admin\_email) | Email for the Coolify admin account. Must be set together with coolify\_admin\_password. If empty, account is created via web UI on first access. | `string` | `""` | no |
...
| <a name="input_install_coolify"></a> [install\_coolify](#input\_install\_coolify) | Whether to install Coolify self-hosted PaaS (https://coolify.io). Mutually exclusive with install\_runtipi. | `bool` | `false` | no |
| <a name="input_install_runtipi"></a> [install\_runtipi](#input\_install\_runtipi) | Whether to install RunTipi homeserver (https://runtipi.io). Mutually exclusive with install\_coolify. | `bool` | `true` | no |
...
| <a name="input_timezone"></a> [timezone](#input\_timezone) | System timezone for the instance (IANA tz database name, e.g. Europe/Rome, UTC, America/New\_York) | `string` | `"Europe/Rome"` | no |
